...
void R_TakeEnvShot( const char *path, const char *name, unsigned maxPixels )
{
	...
	maxSize = min( min( rsh.width, rsh.height ), glConfig.maxTextureSize );
	...
	Vector4Set( rn.viewport, fd.x, rsh.height - size - fd.y, size, size );
	Vector4Set( rn.scissor, fd.x, rsh.height - size - fd.y, size, size );
