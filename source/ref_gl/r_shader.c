...
// Test-branch stub. The 2.15.1-backported shader pipeline lacks the
// inline-text parser the q1/q2 BSP loader expects; forward to the
// name-based loader so q1/q2 maps link and load with default-style
// surfaces. The bisect target is q3 perf, so this is acceptable.
shader_t *R_LoadShaderText( const char *name, shaderType_e type, bool forceDefault, const char *text )
{
	(void)text;
	return R_LoadShader( name, type, forceDefault );
}
