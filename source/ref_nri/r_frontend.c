...
// Destroy the per-swapchain-image attachments (back buffer, pogo buffers + depth image/view). Kept
...
// Create the per-swapchain-image attachments (back buffer, pogo buffers + depth image/view). These are
// all sized to the render (mode) resolution, not the swapchain; the back buffer is letterbox-blitted
// into the screen-res swapchain at present.
	...
	bool verbose )
{
	...
	rsh.shadowSamplerDescriptor = RIDescriptorSampler( &rsh.device, R_ResolveSamplerDescriptor( IT_DEPTHCOMPARE | IT_SPECIAL | IT_DEPTH ) );
...
void RF_Shutdown( bool verbose )
{
	...
	// drain any in-flight frame work before tearing the timeline down
	RITimelineWait( &rsh.device, &rsh.frameTimeline, RITimelinePending( &rsh.frameTimeline ) );
	FreeRITimeline( &rsh.device, &rsh.frameTimeline );
...
void RF_EndFrame( void )
{
...
#if ( DEVICE_IMPL_VULKAN )
	{
		...
		// aspect ratio. The image is centred and the surrounding bars are cleared to black.
		{
			...
			toTransfer[0].dstStageMask = VK_PIPELINE_STAGE_2_BLIT_BIT | VK_PIPELINE_STAGE_2_COPY_BIT;
		...
		// A pending screenshot reads the mode-res back buffer, which the barrier above has already left
		// in COPY_SRC. Capturing it before the blit keeps the saved image free of the letterbox bars.
		const bool captured = R_CaptureRecordScreenshot( &rsh.frame.handle, &rsh.backBuffer[rsh.swapchainIndex], rsh.renderWidth, rsh.renderHeight );
