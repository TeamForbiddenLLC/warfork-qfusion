...
static pack_t *FS_LoadPakFile( const char *packfilename, bool silent )
{
	...
	FILE *fin = NULL;
	int *checksums = NULL;
	char *p, *names, dirName[IDPAKENTRY_NAME_SIZE];
	const char *ext;
	packfile_t *file;
	pack_t *pack = NULL;
	// c++
	...
	if( !vfsHandle )
	{
		...
		if( handle == NULL )
		{
			if( !silent ) Com_Printf( "Error locking PAK file: %s\n", packfilename );
			goto error;
		}
	}
	...
	if( fin == NULL )
	{
		if( !silent ) Com_Printf( "Error opening PAK file: %s\n", packfilename );
		goto error;
	}

	if( fseek( fin, Sys_VFS_FileOffset( vfsHandle ), SEEK_SET ) != 0 )
		goto error;
	...
	if( fseek( fin, Sys_VFS_FileOffset( vfsHandle ) + header.dirofs, SEEK_SET ) != 0 )
		goto error;
	...
	pack = ( pack_t* )FS_Malloc( (int)( sizeof( pack_t ) + ( numFiles + numDirs ) * sizeof( packfile_t ) + namesLen ) );
	pack->filename = FS_CopyString( packfilename );
	pack->files = ( packfile_t * )( ( uint8_t * )pack + sizeof( pack_t ) );
	...
	for( i = 0, file = pack->files; i < numFiles + numDirs; i++, file++ )
	{
		...
		// entries are stored uncompressed and their offsets are absolute, so
		// they never need the zip local header fixup done by FS_PK3CheckFileCoherency
	...
	if( !pack->checksum )
	{
		if( !silent ) Com_Printf( "Couldn't generate checksum for pak file: %s\n", packfilename );
		goto error;
	}
